...
ctest -VV -S $HOME/girder/plugins/wt_data_manager/cmake/circle_continuous.cmake
if [ -f $HOME/build/test_failed ] ; then
	exit 1
fi

cd $HOME/girder_wt_data_manager/wt_data_manager
cp -r ~/build/coverage.xml $HOME/girder_wt_data_manager/wt_data_manager/
